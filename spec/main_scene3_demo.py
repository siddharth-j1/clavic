...
2-phase task (no separate hold):
  Phase 1 (0-6 s) : carry mug upright from start to goal, avoid obstacle
  Phase 2 (6-10 s): pour -- tilt 90 deg about Y axis, hold position at goal

Compiler implicit cost: each K_ii < 200 N/m when arm is near human (goal).
CGMS guarantee K = Q^T Q > 0 is maintained throughout.

3 publication-quality plots (PDF + PNG, 300 dpi):
  1. scene3_workspace.pdf   -- Franka-frame 3D view (X=depth, Y=height, Z=table)
...
HUMAN_POS      = GOAL
HUMAN_PROX_RAD = 0.12
K_AXIS_LIMIT   = 100.0   # N/m per axis — target near-human (matches compiler)
...
def print_diagnostics(trace, best_cost):
    ...
    if np.any(near_mask):
        kn = K_diag[near_mask]
        print(f"  Near-human K (d < {HUMAN_PROX_RAD} m):")
...
def plot_3d_workspace(trace, best_cost, base="scene3_workspace"):
    ...
    # ── axes labels  (X-Z = table plane bottom, Y = height) ──
    # matplotlib X = world x (depth/forward)
    # matplotlib Y = world y (lateral along table)
    # matplotlib Z = world z (height — vertical axis)
    ax.set_xlabel("X — forward/depth (m)", fontsize=9, labelpad=7)
    ax.set_ylabel("Z — lateral/table (m)",  fontsize=9, labelpad=7)
    ax.set_zlabel("Y — height (m)",         fontsize=9, labelpad=7)
    ax.tick_params(labelsize=8)
    # view: look from above-front so X-Y table plane (= X-Z real) is visible at bottom
...
def plot_stiffness(trace, best_cost, base="scene3_stiffness"):
    ...
    near   = d_goal < HUMAN_PROX_RAD

    fig, ax = plt.subplots(figsize=(9, 4.2))

    # near-human shading
    if np.any(near):
        starts = np.where(np.diff(near.astype(int)) ==  1)[0]
        ends   = np.where(np.diff(near.astype(int)) == -1)[0]
        if near[0]:  starts = np.concatenate([[0], starts])
        if near[-1]: ends   = np.concatenate([ends, [len(near)-1]])
        for i, (s, e) in enumerate(zip(starts, ends)):
            ax.axvspan(t[s], t[e], alpha=0.11, color=C_HUMAN,
                       label="Near human" if i == 0 else None, zorder=0)
    ...
    ax.autoscale(axis="y")
    yhi = max(Kd.max() * 1.08, K_AXIS_LIMIT * 1.3)
    ax.set_ylim(0, yhi)
...
def main():
    ...
    policy    = MultiPhaseCertifiedPolicy(taskspec.phases, K0=400.0, D0=30.0)
    ...
    print(f"  Per-axis K_ii < {K_AXIS_LIMIT:.0f} N/m within {HUMAN_PROX_RAD} m of human")
    ...
    print("Scene 3 done -- 3 plots saved as PDF + PNG.")
