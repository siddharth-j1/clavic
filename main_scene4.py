"""
Scene 4: Carry → Hold-at-Waypoint (2 s) → Continue to Goal.
...
def print_diagnostics(trace, best_cost):
    ...
    print("=" * (48*2 + len(" SCENE 4 DIAGNOSTICS ")))
...
#  PLOT 1 — 3D workspace
# ======================================================================
def plot_3d_workspace(trace, best_cost, base="scene4_workspace"):
    pos = trace.position
    t   = trace.time
...
# ======================================================================
def plot_2d_topdown(trace, best_cost, base="scene4_topdown"):
    pos = trace.position
    t   = trace.time
...
# ======================================================================
def plot_stiffness(trace, best_cost, base="scene4_stiffness"):
    K_arr = trace.gains["K"]
...
# ======================================================================
def plot_orientation_euler(trace, best_cost, base="scene4_orientation"):
...
# ======================================================================
def plot_kinematics(trace, best_cost, base="scene4_kinematics"):
    pos   = trace.position
    vel   = trace.velocity
    t     = trace.time
    ...
    fig.suptitle("Scene 4 — Per-axis Position & Velocity", fontsize=12, y=1.01)
    plt.tight_layout()
    plt.savefig(f"{base}.png", dpi=300, bbox_inches="tight", facecolor="white")
    print(f"Saved: {base}.png")
    plt.close()
...
#  CSV export
# ======================================================================
def save_trajectory_csv(trace, csv_path="scene4_trajectory.csv"):
    import csv
    pos   = trace.position
    vel   = trace.velocity
    K_arr = trace.gains["K"]
...
# ======================================================================
def main():
    taskspec = load_taskspec_from_json("spec/scene4_task.json")
    ...
    # avoidance="HARD":
    #   Layer 1 — DMP repulsive forcing inside ODE (strength=0.05, infl=2.0×r)
    #             steers spring-damper attractor organically around obstacle
    ...
    print(f"Scene 4: Carry → Hold-2s → Continue (HARD obstacle avoidance)")
    ...
    # Scene 4 goal is a delivery point — NOT a human.
    # No stiffness reduction near goal: pass human_position=None.
    ...
    save_checkpoint(best_theta, taskspec.horizon_sec, best_cost, trace_final,
                    checkpoint_path="scene4_checkpoint.npz")
    save_trajectory_csv(trace_final, csv_path="scene4_trajectory.csv")
    ...
    print("Scene 4 done — 5 plots saved as PNG (no PDF).")
